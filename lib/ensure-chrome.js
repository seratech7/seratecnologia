...
function ensureChrome() {
  ...
  console.log('[chrome] Baixando Chrome via puppeteer install.mjs...');
  try {
    var installer = path.join(require.resolve('puppeteer/package.json'), '..', 'install.mjs');
    execSync('node ' + installer, { stdio: 'inherit', timeout: 300000, cwd: __dirname });
    found = findChrome();
