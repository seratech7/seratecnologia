...
function findChromeInPuppeteerCache() {
  ...
  var versions = fs.readdirSync(chromeDir).filter(function(d) { return d.startsWith('win64-') || d.startsWith('linux-') || d.startsWith('mac-'); });
  versions.sort().reverse();
  for (var i = 0; i < versions.length; i++) {
    var exe = path.join(chromeDir, versions[i], 'chrome-win64', 'chrome.exe');
    if (fs.existsSync(exe)) return exe;
    exe = path.join(chromeDir, versions[i], 'chrome-win64', 'chrome');
    if (fs.existsSync(exe)) return exe;
    exe = path.join(chromeDir, versions[i], 'chrome-mac', 'chrome');
    if (fs.existsSync(exe)) return exe;
  }
  return '';
}
