...
async function ensureChromeForPuppeteer() {
  ...
  log('Chrome nao encontrado, baixando...');
  try {
    ...
    log('Chrome instalado em: ' + result.executablePath);
    process.env.PUPPETEER_EXECUTABLE_PATH = result.executablePath;
    return result.executablePath;
  } catch (e) {
    log('Falha ao baixar Chrome: ' + e.message);
    return '';
  }
}
...
async function initClient(accountId, sessionId) {
  ...
  try {
    ...
    var chromePath = await ensureChromeForPuppeteer();
