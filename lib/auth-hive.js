...
const SESSION_TTL_INACTIVE = (parseInt(process.env.AUTH_TTL_INACTIVE_MINUTES, 10) || 8 * 60) * 60 * 1000;
...
async function hashPassword(password) {
  ...
  const pepperedB64 = peppered.toString('base64');
  ...
  return bcrypt.hashSync(pepperedB64, 12);
}
...
async function verifyPassword(password, storedHash) {
  ...
  if (storedHash.startsWith('$argon2id$') || storedHash.startsWith('$argon2i$') || storedHash.startsWith('$argon2d$')) {
    if (argon2) {
      try {
        const verified = await argon2.verify(storedHash, peppered);
        return { verified: verified, needsRehash: false };
      } catch (e) {
...
function generateFakeHash() {
  return bcrypt.hashSync(crypto.randomBytes(32).toString('base64'), 12);
}
...
// Em vez de derrubar a sessão quando o IP/zona ou User-Agent muda (rede móvel,
// IP dinâmico, atualização de navegador), faz o re-bind: atualiza os dados de
// binding e mantém o usuário logado. Sessão continua válida.
function rebindSession(sidHash, req) {
  const currentUaHash = hashUserAgent(req.get('User-Agent'));
  const currentIpZone = getIpZone(req.ip || req.connection.remoteAddress || '');
  db.updateAuthSessionBinding(sidHash, currentUaHash, currentIpZone);
}
...
function isHttps(req) {
  return !!(req && (req.secure || (req.headers && req.headers['x-forwarded-proto'] === 'https') || (req.headers && req.headers['x-forwarded-proto'] && req.headers['x-forwarded-proto'].split(',')[0].trim() === 'https')));
}

// O cookie de sessao auth usa o prefixo __Host-, que OBRIGA o atributo Secure.
// Se servirmos em HTTPS mas o NODE_ENV nao for 'production', o navegador
// REJEITA o cookie (prefixo __Host- sem Secure) e o usuario e deslogado em
// toda nova pagina. Por isso o 'secure' deve seguir o protocolo real (HTTPS),
// nao o NODE_ENV. Em HTTP (dev local) usamos nome sem prefixo.
function getSessionCookieName(req) {
  return isHttps(req) ? '__Host-session' : 'sera_session';
}
...
function clearSessionCookie(res, req) {
  const cookieName = getSessionCookieName(req);
  const opts = { path: '/', httpOnly: true, secure: isHttps(req), sameSite: 'lax' };
  if (cookieName.indexOf('__Host-') === 0) opts.domain = undefined;
...
function clearDeviceCookie(res, req) {
  res.clearCookie(DEVICE_ID_COOKIE, { path: '/', httpOnly: true, secure: isHttps(req), sameSite: 'lax' });
}
...
function validateSession(req, res) {
  ...
  if (!checkSessionBinding(session, req)) {
    // IP/zona ou User-Agent mudou (rede móvel, IP dinâmico). Faz re-bind em vez
    // de deletar — o usuário não é deslogado por mudança de rede/navegador.
    rebindSession(sidHash, req);
    db.logAuthEvent(session.user_uid, 'session_rebound', req.ip || '', req.get('User-Agent') || '', 'success', 'binding_rebound');
  }
...
function requireAuth(userType) {
  return (req, res, next) => {
    ...
    if (!session) {
      ...
      const loginPath = userType === 'admin' ? '/admin/login' : (userType === 'seller' ? '/seller/login' : '/login');
