  ...
  `);
    ...
    try {
      ...
      if (_sc.length > 0) {
        ...
        if (_scCols.indexOf('payment_status') === -1) db.run("ALTER TABLE digital_sales ADD COLUMN payment_status TEXT");
        if (_scCols.indexOf('mp_preference_id') === -1) db.run("ALTER TABLE digital_sales ADD COLUMN mp_preference_id TEXT");
        if (_scCols.indexOf('mp_payment_id') === -1) db.run("ALTER TABLE digital_sales ADD COLUMN mp_payment_id TEXT");
        if (_scCols.indexOf('paid_at') === -1) db.run("ALTER TABLE digital_sales ADD COLUMN paid_at TEXT");
      }
    } catch (e) {}
...
// Reserva atômica: só marca como vendido se ainda estiver disponível.
// Retorna true somente se esta requisição conseguiu reservar (evita venda duplicada).
function claimDigitalStock(stockId, saleId) {
  run("UPDATE digital_stock SET status = 'sold', sale_id = ? WHERE id = ? AND status = 'available'", [saleId, stockId]);
  var row = get("SELECT status FROM digital_stock WHERE id = ?", [stockId]);
  return !!(row && row.status === 'sold');
}

// Libera reservas abandonadas (compra iniciada mas nunca paga) após X minutos,
// devolvendo o estoque para disponível e marcando a venda como expirada.
function releaseExpiredDigitalReservations(minutes) {
  var mins = minutes || 30;
  var expired = query("SELECT ds.id, ds.stock_id FROM digital_sales ds WHERE ds.status = 'pending_payment' AND ds.created_at <= datetime('now', '-' + mins + ' minutes')") || [];
  expired.forEach(function(s) {
    if (s.stock_id) run("UPDATE digital_stock SET status = 'available', sale_id = NULL WHERE id = ? AND status = 'sold'", [s.stock_id]);
    run("UPDATE digital_sales SET status = 'expired', payment_status = 'expired' WHERE id = ?", [s.id]);
  });
  return expired.length;
}
function createDigitalSale(data) {
  run("INSERT INTO digital_sales (product_id, stock_id, buyer_name, buyer_email, buyer_phone, delivery_channel, delivery_contact, observation, price, delivery_code, status, payment_status, mp_preference_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [data.product_id, data.stock_id || null, data.buyer_name || '', data.buyer_email || '', data.buyer_phone || '', data.delivery_channel || '', data.delivery_contact || '', data.observation || '', data.price || 0, data.delivery_code, data.status || 'pending', data.payment_status || 'pending', data.mp_preference_id || null]);
  var r = get("SELECT MAX(id) as id FROM digital_sales");
  return r ? r.id : null;
}
function getDigitalSaleById(id) {
  return get("SELECT ds.*, dp.name as product_name, dp.image as product_image, dsk.credential, dsk.password, dsk.note FROM digital_sales ds LEFT JOIN digital_products dp ON ds.product_id = dp.id LEFT JOIN digital_stock dsk ON ds.stock_id = dsk.id WHERE ds.id = ?", [id]);
}
function updateDigitalSalePayment(saleId, opts) {
  var sets = [];
  var params = [];
  if (opts.payment_status) { sets.push('payment_status = ?'); params.push(opts.payment_status); }
  if (opts.status) { sets.push('status = ?'); params.push(opts.status); }
  if (opts.mp_payment_id) { sets.push('mp_payment_id = ?'); params.push(opts.mp_payment_id); }
  if (opts.mp_preference_id) { sets.push('mp_preference_id = ?'); params.push(opts.mp_preference_id); }
  if (opts.paid_at) { sets.push('paid_at = ?'); params.push(opts.paid_at); }
  if (!sets.length) return;
  params.push(saleId);
  run('UPDATE digital_sales SET ' + sets.join(', ') + ' WHERE id = ?', params);
}
function getDigitalSaleByPreference(prefId) {
  return get("SELECT * FROM digital_sales WHERE mp_preference_id = ?", [prefId]);
}
function getDigitalSaleByDeliveryCode(code) {
  return get("SELECT ds.*, dp.name as product_name, dp.image as product_image, dp.slug as slug, dsk.credential, dsk.password, dsk.note FROM digital_sales ds LEFT JOIN digital_products dp ON ds.product_id = dp.id LEFT JOIN digital_stock dsk ON ds.stock_id = dsk.id WHERE ds.delivery_code = ?", [code]);
}
